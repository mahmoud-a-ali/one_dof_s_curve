...
int main(int argc, char **argv)
{
    ...
    one_dof_scurve_coef(P_wpt, ref_T, traj_T, jrk, seg_idx, sm, vm, am, jm );
    double T_tot =0;
    for (auto& t : traj_T[3]){
//        ROS_INFO_STREAM( "t: " << t );
        T_tot += t;
    }
    ROS_INFO_STREAM( "total optimal time: " << T_tot );
    ...
    while(tg < T_tot){
        ...
        bool check_sample =  sample_scurve( tg,  traj_T, TPVA,  jrk, seg_idx);
        Ts.push_back(tg);
        Ps.push_back(TPVA[1]);
        Vs.push_back(TPVA[2]);
        As.push_back(TPVA[3]);
